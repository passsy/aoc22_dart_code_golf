void main(List<String> args) {
  ...
  for (final line in args) {
    ...
    final [all, ...] = RegExp('items: (.*)').firstMatch(line)!.groups([1]);
    ...
    final mod =
        int.parse(RegExp(r'divisible by (\d+)').firstMatch(line)!.group(1)!);
    final whenTrueTarget =
        int.parse(RegExp(r'If true:\D*(\d+)').firstMatch(line)!.group(1)!);
    final whenFalseTarget =
        int.parse(RegExp(r'If false:\D*(\d+)').firstMatch(line)!.group(1)!);
    final throwItem =
        (int it) => it % mod == 0 ? whenTrueTarget : whenFalseTarget;
...
class Monkey {
  ...
  void doOperation() {
    while (items.isNotEmpty) {
      ...
      others.firstWhere((m) => m.id == target).items.add(notBroken);
    }
  }
}
